...
String WiFiNm         = "Tha1-2.4G";
String WiFiPwd        = "0819065291";
...
//      Serial.println(char(EEPROM.read(i)));
      ...
      eeprom_wifi += char(EEPROM.read(i));
   }
  }

  SSID = String(eeprom_wifi);
  Serial.println(SSID); 
}
...
//      Serial.println(char(EEPROM.read(i)));
      ...
      eeprom_wifipwd += char(EEPROM.read(i));
   }
  }

  PASSWORD = String(eeprom_wifipwd);
...
//    Serial.println(BrdNm[x]); 
  ...
  // เคลียร์พื้นที่ก่อน เขียน WiFiNm
  for(int i=20;i < 40;i++)
  {
   EEPROM.write(i, 0);
  }
  ...
  // เขียน WiFiNm
//  Serial.println("------------------Show : WiFiNm ------------------"); 
  Serial.print("WiFiName : ");
  Serial.println(WiFiNm); 
...
//    Serial.println(BrdNm[x]); 
    ...
    if(x < WiFiNm.length())EEPROM.write(i, WiFiNm[x]); //Write one by one with starting address of 0x0F
    x++;
  }
  SSID = String(WiFiNm);

  // เคลียร์พื้นที่ก่อน เขียน WiFiPwd
  for(int i=20;i < 40;i++)
  {
   EEPROM.write(i, 0);
  }
...
//    Serial.println(BrdNm[x]); 
    ...
    if(x < WiFiPwd.length())EEPROM.write(i, WiFiPwd[x]); //Write one by one with starting address of 0x0F
    x++;
  }
  PASSWORD = String(WiFiPwd);
